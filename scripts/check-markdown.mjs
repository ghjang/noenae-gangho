...
{
  const d = fromMarkdown(['# 비급', '', '- 뿌리', '  - 자식 갑', '    - 손주', '  - 자식 을', '- 둘째 뿌리'].join('\n'))
...
// 3) ↻(순환 재방문 표시) 줄은 건너뛴다 — 같은 쪽지가 다시 태어나면 안 됨
{
  ...
  ok(d?.nodes.length === 2 && d?.edges.length === 1, `순환 표시: 쪽지 2·緣 1 기대, 실제 ${d?.nodes.length}·${d?.edges.length}`)
}

// 4) 빈 쪽지 라벨 환원 — 두 팩의 mdEmptyNode 모두 빈 글로
{
  const d = fromMarkdown('- (빈 쪽지)\n- (빈 노트)', ['(빈 쪽지)', '(빈 노트)'])
  ok(d?.nodes.every((n) => n.text === ''), '빈 쪽지 라벨은 빈 글로 돌아와야')
}
...
// 6) 이상 들여쓰기(깊이 점프) — 무너지지 않고 緣 양끝이 늘 실존
{
  ...
  ok(d.edges.every((e) => ids.has(e.a) && ids.has(e.b)), '깊이 점프: 緣 양끝 실존해야')
