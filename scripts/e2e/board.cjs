...
(async () => {
  ...
  await p.waitForTimeout(300);
  ok(
    (await p.locator('.board').count()) === 1 && (await p.locator('.viewport').count()) === 0,
    '토글: 오행진 등장·캔버스 퇴장',
  );
  ok((await p.locator('.board .col').count()) === 5, '오행 5종대');
  ok(
    (await p.locator('.hud').count()) === 0 && (await p.locator('.minimap').count()) === 0,
    '캔버스 전용 UI(HUD/전도) 숨김',
  );
  ...
  ok(
    JSON.stringify(counts) === JSON.stringify(['2', '1', '1', '1', '0']),
    `종대 머릿수 [2,1,1,1,0] (실측 ${counts})`,
  );
  ...
  ok(
    mukCards[0].includes('빈 쪽지') && mukCards[1].includes('깨다름'),
    `정렬 y→x: 빈 쪽지(위) → 깨다름 (실측 ${mukCards})`,
  );
  ...
  ok(
    (await p.locator('.board .col:first-child h3 i.cur').count()) === 1,
    '그 종대 머리 동그라미에 .cur 링 (팔레트 어휘 나침반)',
  );
  ...
  await p.keyboard.press('ArrowRight');
  ok(
    (await p.locator('.board .col:nth-child(1) .card.sel').count()) === 1,
    '→ 빈 남 종대 건너 첫 종대로 순환',
  );
  ...
  ok(
    (await p.locator('.node.selected').textContent()).includes(jumpTxt),
    'Enter 점프: 그 쪽지가 선택돼 있다',
  );
  await p.locator('button[aria-label="오행진"]').click();
  await p.waitForTimeout(300);
  ...
  ok(
    (await p.locator('.board .card.sel').getAttribute('data-id')) === selId,
    '낙관(선택)은 제자리 — 금테만 움직인다',
  );
  await p.keyboard.press('Enter');
  ...
  const colScroll = await p
    .locator('.board .col:first-child .cards')
    .evaluate((el) => el.scrollHeight >= el.clientHeight);
  const cardH = await p
    .locator('.board .col:first-child .card')
    .first()
  ...
  ok(
    cWrap[0] === '0' && cWrap[4] === '1',
    `Ctrl+← 첫 종대에서 끝(남)으로 순환 — 빈 종대도 목적지 (실측 ${cWrap})`,
  );
  await p.keyboard.press('Control+z');
  await p.waitForTimeout(600);
  ...
  await p.waitForTimeout(300);
  ok(
    (await p.$$eval('.board .col h3 em', (els) => els.map((e) => e.textContent)))[0] === '0',
    'Delete → 선택 카드 베기',
  );
  await p.keyboard.press('Control+z');
  await p.waitForTimeout(300);
  ok(
    (await p.$$eval('.board .col h3 em', (els) => els.map((e) => e.textContent)))[0] === '1',
    'Ctrl+Z → 귀환',
  );
  ...
  await p.waitForTimeout(300);
  ok(
    (await p.locator('.board .card .fold').textContent()) === '▸1',
    '접힌 카드에 ▸1 배지 (직계 수 — 캔버스와 같은 표기)',
  );
  {
    const cb = await p.locator('.board .card', { hasText: 'difference' }).boundingBox();
    const fb = await p.locator('.board .card .fold').boundingBox();
    ok(
      fb && fb.x >= cb.x && fb.x + fb.width <= cb.x + cb.width + 1 && fb.y >= cb.y,
      '배지가 카드 안에 실재 (순간이동·투명화 보초)',
    );
  }
  await p.locator('.board .card', { hasText: 'difference' }).dblclick();
  await p.waitForTimeout(400);
  ok(
    (await p.locator('.node').count()) === foldedNodes,
    `접힌 카드 점프 → 봉문 보존 (쪽지 ${foldedNodes} 그대로)`,
  );
  ...
  ok(
    mukAfterAdd.length === 2 && mukAfterAdd[1].includes('빈 쪽지'),
    `보드 + → 먹 종대 끝에 새 쪽지 (실측 ${mukAfterAdd.length}장)`,
  );
  await p.keyboard.press('Control+z');
  await p.waitForTimeout(300);
