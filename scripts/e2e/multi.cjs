...
;(async () => {
  ...
  const pos = (text) => page.locator('.node', { hasText: text }).first().evaluate((el) => [parseFloat(el.style.left), parseFloat(el.style.top)])
  ...
  ok(Math.round(ax1 - ax0) === 60 && Math.round(ay1 - ay0) === 40, `그룹 드래그: 잡은 쪽지 +60/+40 (${Math.round(ax1 - ax0)},${Math.round(ay1 - ay0)})`)
  ...
  await page.waitForTimeout(120)
  const colors = await page.$$eval('.node', (els) => els.map((el) => [el.textContent.includes('깨다름') || el.textContent.includes('difference'), el.dataset.color]))
  ok(colors.filter(([m]) => m).every(([, c]) => c === 'nam'), '그룹 색칠: 무리 둘 다 남(nam)')
  await page.keyboard.press('Control+z')
  await page.waitForTimeout(120)
