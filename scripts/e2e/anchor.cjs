...
(async () => {
  ...
  const checkAll = async (label, sel = 'svg.edges:not(.lift-layer) path.vis') => {
    ...
    ok(
      rep.length > 0 && rep.every((r) => r.onSide && !r.inside),
  ...
  for (let i = 1; i <= 4; i++)
    await page.mouse.move(nb.x + nb.width / 2 + i * 15, nb.y + nb.height / 2 + i * 12);
  ...
  await page.screenshot({
    path: '/tmp/anchor-drag.png',
    clip: { x: Math.max(0, db.x - 260), y: Math.max(0, db.y - 160), width: 620, height: 400 },
  });
  await page.mouse.up();
