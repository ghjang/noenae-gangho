...
;(async () => {
  ...
  // ── 폰 화면 (390×844, 아이폰급) ──
  const mob = await (await browser.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 2, hasTouch: true })).newPage()
  ...
  ok(palBB.y >= titleBB.y + titleBB.height - 2, `팔레트가 둘째 줄로 (title y${Math.round(titleBB.y)} → palette y${Math.round(palBB.y)})`)
  ok(Math.round(barBB.height) === 86, `바 높이 = --bar-h 86 (실측 ${Math.round(barBB.height)})`)
  const vpBB = await mob.locator('.viewport').boundingBox()
  ok(Math.round(vpBB.y) === Math.round(barBB.height), `뷰포트 상단 == 바 하단 (틈/겹침 없음, ${Math.round(vpBB.y)})`)
  ...
  ok((await mob.evaluate(() => getComputedStyle(document.documentElement).touchAction)) === 'manipulation', 'html touch-action: manipulation (더블탭 줌 차단)')
  ok((await mob.locator('.viewport').evaluate((el) => getComputedStyle(el).touchAction)) === 'none', '캔버스 touch-action: none 유지 (자체 핀치)')
  ok((await mob.locator('.bar button').first().evaluate((el) => getComputedStyle(el).userSelect)) === 'none', '버튼 user-select: none (꾹 눌러도 선택 안 됨)')
  ...
  // ── 데스크톱 회귀 (1280) ──
  const desk = await (await browser.newContext({ viewport: { width: 1280, height: 800 }, deviceScaleFactor: 2 })).newPage()
