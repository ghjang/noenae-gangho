...
(async () => {
  ...
  ok(
    body.includes('data:image/svg+xml') && / 3,? 3,? .*default/.test(body.replace(/"/g, '')),
    `body: 커스텀 화살표 + 핫스팟 3 3 (${body.slice(-40)})`,
  );
  const btn = await cur('.bar button');
  ok(btn.includes('data:image') && /11,? 3/.test(btn), 'bar 버튼: 손가락 + 핫스팟 11 3');
  const vp = await cur('.viewport');
  ok(
    vp.includes('data:image') && /12,? 12/.test(vp) && vp.includes('grab'),
    '뷰포트: 손바닥 + 12 12 (grab 폴백)',
  );
  ...
  ok(
    ta.includes('data:image') && /12,? 12/.test(ta) && ta.includes('text'),
    '편집 textarea: I빔 커스텀 + 12 12',
  );
  ...
  await page.evaluate((sizes) => {
    ...
    sheet.style.cssText =
      'position:fixed;inset:0;z-index:99999;display:grid;grid-template-rows:1fr 1fr;font:12px sans-serif';
