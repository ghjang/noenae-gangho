...
;(async () => {
  ...
  ok(preText.includes('"app": "noenae-gangho"') || preText.includes('"app":"noenae-gangho"'), 'pre 본문에 원문 보존')
  ...
  ok(Math.abs(tb.x - pb.x) < 1 && Math.abs(tb.y - pb.y) < 1 && Math.abs(tb.width - pb.width) < 1, 'overlay 박스 정렬 일치')
  // 긴 내용 스크롤 동기화
  await ta.fill(Array.from({ length: 80 }, (_, i) => `- 쪽지 ${i}`).join('\n'))
  await ta.evaluate((el) => { el.scrollTop = 300; el.dispatchEvent(new Event('scroll')) })
  await page.waitForTimeout(100)
