...
(async () => {
  ...
  ok(
    await p.evaluate(
      () => Object.keys(localStorage).filter((k) => k.startsWith('noenae-gangho-doc-')).length === 1,
    ),
    'B 벤 문서의 본문 키도 제거',
  );
  await p.keyboard.press('Escape');
  ...
  // ── C. 인덱스 유실 복구 + 가져오기 격리 ──
  {
    ...
    await ctx2.addInitScript(() => {
      ...
      if (!localStorage.getItem('noenae-gangho-docs')) {
        ...
        localStorage.setItem(
          'noenae-gangho-doc-x1',
          doc([{ id: 'p', x: 80, y: 80, text: '고아 일권', color: 'muk' }]),
        );
        localStorage.setItem(
          'noenae-gangho-doc-x2',
          doc([
