...
// 반환: [{ c, t }] — c는 토큰 클래스, t는 원문 그대로.
// 이어 붙이면 입력과 1글자도 다르지 않다(편집 overlay 정렬의 전제).
// ──────────────────────────────────────────────

export type TokenClass = 'key' | 'str' | 'num' | 'kw' | 'pun' | 'head' | 'bullet' | 'cyc' | '';
...
export function highlightJson(text: string): Token[] {
  const s = String(text);
  const out: Token[] = [];
  let last = 0;
...
export function highlightMd(text: string): Token[] {
  const out: Token[] = [];
...
export function highlightAuto(text: string): Token[] {
