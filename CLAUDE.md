...
- 색 5종은 오행 체계: `muk/cheong/dan/hwang/nam` — 추가·변경 시 `COLORS`(store) · `app.css`(`:root`의 `--c-*` 변수와 `.node[data-color]` 규칙 두 군데) · `COLOR_LABEL`(App) 동기화
- 조작·단축키를 바꾸면 App 안 도움말 카드와 README '조작 요결' 표도 같이 갱신
