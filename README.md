...
| 족보에서 화살표 · Enter · Delete | ↑↓ 행 순회(양끝 순환) · ← 펼친 가지면 접기, 아니면 부모로 · → 접힌 가지면 펼치기, 아니면 첫 자식으로 · 3 = 선택 가지로 파고들기 · Esc = 부모 가지로 한 단 위 · 0 = 전체 족보로 직행 (크럼 = 조상 경로, 조상 클릭 = 그 가지로) · Enter = 조준 선택→캔버스 점프 · Delete = 베기 |
