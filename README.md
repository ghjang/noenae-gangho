...
상단 바(왼→오 = 문서 → 창작 → 입출력 → 위험 → 메타): 서가(겹책) 아이콘(여러 강호 — 갈아타기/개창/이름 고치기/베기, 문서별 저장·뷰) · 오행진/캔버스 토글(색별 종대 칸반 — 읽기 전용, 정렬은 캔버스 y→x, 카드 더블클릭=캔버스 점프, 모드는 문서별 저장) · 오행 색 팔레트(선택 시 칠하기 · 빈손 클릭 = 붓 색 고르기 — 링 표시가 지금 붓 색 · 호버 = 같은 색 비추기) · +(새 쪽지 — 지금 붓 색으로 핀다) · M↓ 마크(비급.md — 마크다운 개요 출력) · 상자↑ 아이콘(내보내기 — JSON) · 받기↓ 아이콘(불러오기 — JSON/비급.md 붙여넣기, 양식 자동 판별) · 빗자루 아이콘(강호 비우기 — 확인 카드, Ctrl+Z로 복구 가능) · 封/武 무공봉인 토글(무협 ↔ 일반 말투 전환).
