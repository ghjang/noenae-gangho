...
| F2 (또는 Ctrl+Enter) | 선택한 쪽지 편집 — 편집 중 입력이 비면 Backspace로 그 쪽지째 베기(아웃라이너 국룰, 갓 친 빈 念 무르기) |
...
| 1 · 2 · 3 | 뷰 직행 — 1 캔버스 · 2 오행진 · 3 족보. 1·2·3 모두 선택 念을 화면 중앙에 비춘다(시야만, 시야를 좁히지 않음). 시야를 좁혀 선택 가지에 집중하려면 Shift+3 |
| 족보(아웃라인) 뷰 | 위계(트리)로 읽기 — 선택 念은 화면 중앙에 비춰 열림(Shift+3 = 그 가지로 집중/스코프) · 행 클릭=선택 · 더블클릭=캔버스 점프 · ▸/▾=봉문 토글 · ↻=재방문(순환·다중 부모) · Ctrl+F=검색 필터 |
| 족보에서 화살표 · Space · Enter · Delete | ↑↓ 행 순회(양끝 순환) · ← 펼친 가지면 접기, 아니면 부모로 · → 접힌 가지면 펼치기, 아니면 첫 자식으로 · Space·C = 봉문/개문(하위 있을 때, 캔버스와 동일) · Shift+3 = 선택 가지로 집중(스코프) · Esc = 부모 가지로 한 단 위 · 0 = 전체 족보로 직행 (크럼 = 조상 경로, 조상 클릭 = 그 가지로) · Enter = 조준 선택→캔버스 점프 · Delete = 베기 |
| 족보에서 Ctrl+F (검색 필터) | 상단 인라인 검색창 — 입력에 따라 매칭 ∪ 조상 경로만 남긴다(접힌 가지 속 매칭도 드러냄·접기 멈춤, **스코프 중이면 그 가지 안에서만 검색**) · ↑↓·클릭 = 결과 선택(평소 항법과 동일, 접힌 매칭도 검색 중엔 선택 유지) · Enter = 그 쪽지로 캔버스 점프 · Esc = 닫고 원래 족보 복원(비파괴) |
...
src/
  ...
  OutlineView.svelte   # 족보(아웃라인) 뷰 — 트리 개요 + 스코프·봉문 토글·검색 필터(#154)
